...
class CCKRules extends JAccessRules
{
	...
	public function merge($actions)
	{
		...
		elseif ($actions instanceof CCKRules)
		{
	...
	{
		$var_type	=	( isset( $this->_actions[$action]->var_type ) && $this->_actions[$action]->var_type != '' ) ? $this->_actions[$action]->var_type : 'boolean';
		
		if (isset($this->_data[$action]))
		{
			// If exists, merge the action.
			$this->_data[$action]->mergeIdentities($identities, $var_type);
		}
		else
		{
			// If new, add the action.
			$this->_data[$action] = new CCKRule($identities, $var_type);
		}
	}
	...
	public function allow($action, $identity)
	{
		...
		if (isset($this->_data[$action]))
		{
			return $this->_data[$action]->allow($identity, $var_type);
		}
