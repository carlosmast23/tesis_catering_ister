...
class modFlexBannersHelper {
	public static function FlexBannersQuery($details) {
		...
		if($menu -> getActive() == $menu -> getDefault()) {
			$contentif = "AND IF((select count(distinct id) from `#__flexbanners` 
        				where `#__flexbanners`.`frontpage` = 1)>0,
                       `#__flexbanners`.`id` in (SELECT `#__flexbanners`.`id` FROM `#__flexbanners` 
                       								WHERE `#__flexbanners`.`frontpage` = 1) 
                       								AND `#__flexbanners`.`restrictbyid`=1,
                       `#__flexbanners`.`restrictbyid`=0)";
		} elseif(!is_null($details['contentid']) and $details['categoryid'] == 0 and $details['sectionid'] == 0) {
			$contentif = "AND IF((select count(distinct bannerid) from `#__flexbannersin`
                        where `#__flexbannersin`.`contentid` = " . $details['contentid'] . ")>0,
                       `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` 
                                                    WHERE `#__flexbannersin`.`contentid` = '" . $details['contentid'] . "') 
                                                    AND `#__flexbanners`.`restrictbyid`=1,
                       `#__flexbanners`.`restrictbyid`=0)";

		} elseif(!is_null($details['contentid'])) {
			$contentif = "AND IF((select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`contentid` = " . $details['contentid'] . ")>0,
                       `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`contentid` = '" . $details['contentid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                       IF(`#__flexbanners`.`restrictbyid` and (select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`categoryid` = " . $details['categoryid'] . ")>0,
                          `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`categoryid` = '" . $details['categoryid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                          IF(`#__flexbanners`.`restrictbyid` and (select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`sectionid` = " . $details['sectionid'] . ")>0,
                             `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`sectionid` = '" . $details['sectionid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                             `#__flexbanners`.`restrictbyid`=0)))";

		} elseif(!is_null($details['categoryid'])) {
			$contentif = "AND IF((select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`categoryid` = " . $details['categoryid'] . ")>0,
                        `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`categoryid` = '" . $details['categoryid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                        IF(`#__flexbanners`.`restrictbyid` and (select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`sectionid` = " . $details['sectionid'] . ")>0,
                           `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`sectionid` = '" . $details['sectionid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                           `#__flexbanners`.`restrictbyid`=0))";
		} elseif(!is_null($details['sectionid'])) {
			$contentif = "AND IF((select count(distinct id) from `#__flexbannersin` where `#__flexbannersin`.`sectionid` = " . $details['sectionid'] . ")>0,
                       `#__flexbanners`.`id` in (SELECT `#__flexbannersin`.`bannerid` FROM `#__flexbannersin` WHERE `#__flexbannersin`.`sectionid` = '" . $details['sectionid'] . "') AND `#__flexbanners`.`restrictbyid`=1,
                       `#__flexbanners`.`restrictbyid`=0)";
		} else {
			$contentif = "AND `#__flexbanners`.`restrictbyid`=0";
		}
		return $contentif;
	}
